...
<?php
	...
	$stuid = $student['id'];
	$sql = "SELECT * FROM borrow LEFT JOIN files ON files.id=borrow.book_id WHERE faculty_id = '$stuid' ORDER BY date_borrow DESC";
	$action = '';
	if(isset($_GET['action'])){
		$sql = "SELECT * FROM returns LEFT JOIN files ON files.id=returns.file_id WHERE faculty_id = '$stuid' ORDER BY date_return DESC";
		$action = $_GET['action'];
	}
